...
class model_bl_def_sf:
    ...
    def __init__(self,nb_stitches,time_step,esp,r,mode):
        ...
        self.h_interfaces = np.concatenate(([0],condi_config.levels_boundaries))
        ...
        # On calcule la liste des diamètre nécessaire pour la sédimentation des mailles
        # On calcule l'épaisseur des mailles
        self.epaiss_maille = np.array([self.h_interfaces[stitche+1]-self.h_interfaces[stitche] for stitche in range(nb_stitches)])

        # On associe les diamètres et on part du centre de la maille d'où le décalage
        dist_maille = np.array(Formatage(esp).Epaiss_to_diam(self.h_interfaces))
    ...
    def run(self):
        ...
        for t_time in tqdm(range(self.nb_time_step), bar_format = format_affichage_time, desc = f"Avancement total Box Lagrangien déformable Step_Forward : ", position = 0, colour = 'blue'):
            ...
            for stitch_dep in tqdm(np.where(concentration_profil!=0)[0], bar_format = format_affichage_level, desc = f"Calculs t = {t_time * self.time_step} / {self.nb_time_step * self.time_step} s : ", leave = False, colour = "green"):
                ...
                # On compte le nombre de particule
                nb_part = concentration_profil0[stitch_dep]

                # On regarde ce qui tombe au sol et on l'enregistre
                chute_int = self.Eq_config.calcul_precip(self.h_interfaces[stitch_dep], self.h_interfaces[stitch_dep+1], concentration_profil0[stitch_dep],self.time_step*(t_time+1), Lambda[stitch_dep])
            ...
            chute_tot.append(chute_tot_int-sum(chute_tot))
